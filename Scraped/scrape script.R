...
months <- c("Jan", "February", "Mar", "Apr", "May", "June", "July", "Aug", "Sep", "Oct", "Nov", "Dec")
  ...
  for (j in months) {
    for (k in days) {
      for (m in months2) {
        ...
        fileurl <- paste(url, i, "/", j, "/VSDATA_", i, m, k, ".zip",  sep = "")
        
        filename <- paste("VSDATA_", i, m, k, ".zip", sep = "")
        
        filename2 <- paste("VSDATA_", i, m, k, ".csv", sep = "")
